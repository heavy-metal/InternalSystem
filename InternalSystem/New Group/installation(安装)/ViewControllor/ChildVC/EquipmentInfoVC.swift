...
class EquipmentInfoVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var equipmentArray = [EquipmentModel]()
    lazy var firstSaveArray = [EquipmentModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("EquipmentInfo"), object: nil, queue: nil) { (info) in
...
extension EquipmentInfoVC {
    ...
    fileprivate func getData(_ firstGet:Bool){
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            StockTool.SearchDevice(params: params as? [String : String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    ...
                    self.equipmentArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        VC.equipmentModel = equipmentArray[indexPath.row]
        VC.detailType = .EquipmentInfo
...
extension EquipmentInfoVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = equipmentArray[indexPath.row]
        cell.clientNameLabel.text = model.ClientName
        cell.contactLabel.text = model.Model
        cell.phoneLabel.text = "车牌号:\(model.LicNum)"
...
extension EquipmentInfoVC {
    ...
    fileprivate func setupfooter() {
        tableView.mj_footer = MJRefreshAutoNormalFooter(refreshingBlock: {[weak self] in
            if self?.equipmentArray.count != 0 {
                self?.refreshCount += 1
            }
...
extension EquipmentInfoVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .EquipmentInfo
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.equipmentArray = self.firstSaveArray
