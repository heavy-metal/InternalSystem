...
class ReceiptOrderVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var receiptArray = [ReceiptModel]()
    lazy var firstSaveArray = [ReceiptModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("receiptOrder"), object: nil, queue: nil) { (info) in
...
extension ReceiptOrderVC {
    ...
    fileprivate func getData(_ firstGet:Bool){
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            StockTool.SearchInOrder(params: params as? [String:String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    self.receiptArray = array
                    if self.firstSaveArray.count == 0 {self.firstSaveArray = array}
                }else{
                    self.receiptArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let VC = ReceiptOrderDetailVC()
        VC.receiptModel = receiptArray[indexPath.row]
...
extension ReceiptOrderVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = receiptArray[indexPath.row]
        cell.clientNameLabel.text = model.ProdNames
        cell.contactLabel.text = model.InOrderNo
...
extension ReceiptOrderVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .receiptOrder
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.receiptArray = self.firstSaveArray
