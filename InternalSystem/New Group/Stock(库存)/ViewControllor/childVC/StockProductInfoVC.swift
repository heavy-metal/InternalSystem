...
class StockProductInfoVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var StockProductArray = [StockProductModel]()
    lazy var firstSaveArray = [StockProductModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("stockProductInfo"), object: nil, queue: nil) { (info) in
...
extension StockProductInfoVC {
    fileprivate func getData (_ firstGet:Bool) {
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            StockTool.SearchProduct(params: params as? [String:String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    self.StockProductArray = array
                    if self.firstSaveArray.count == 0 {self.firstSaveArray = array}
                }else{
                    self.StockProductArray.append(contentsOf: array)
                }
...
extension StockProductInfoVC {
    ...
    fileprivate func setupfooter() {
        tableView.mj_footer = MJRefreshAutoNormalFooter(refreshingBlock: {[weak self] in
            if self?.StockProductArray.count != 0 {
                self?.refreshCount += 1
            }
...
extension StockProductInfoVC{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = SalsesDetailVC()
        vc.detailType = .stockProductDetail
        vc.stockProductModel = StockProductArray[indexPath.row]
        self.navigationController?.pushViewController(vc, animated: true)
    }
}
extension StockProductInfoVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .stockProductInfo
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.StockProductArray = self.firstSaveArray
