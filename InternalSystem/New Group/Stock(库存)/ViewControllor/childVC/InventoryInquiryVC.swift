...
class InventoryInquiryVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var inventoryArray = [InventoryModel]()
    lazy var firstSaveArray = [InventoryModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("InventoryInquiry"), object: nil, queue: nil) { (info) in
...
extension InventoryInquiryVC {
    ...
    fileprivate func getData(_ firstGet:Bool){
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            StockTool.SearchStock(params: params as? [String : String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    ...
                    self.inventoryArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        VC.inventoryModel = inventoryArray[indexPath.row]
        VC.productType = .InventoryInquiry
...
extension InventoryInquiryVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = inventoryArray[indexPath.row]
...
extension InventoryInquiryVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .InventoryInquiry
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.inventoryArray = self.firstSaveArray
