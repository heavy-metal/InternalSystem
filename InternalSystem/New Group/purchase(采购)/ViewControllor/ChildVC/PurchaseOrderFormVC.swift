...
class PurchaseOrderFormVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var purchaseArray = [PurchaseModel]()
    lazy var firstSaveArray = [PurchaseModel]()
    ...
    override func viewDidLoad() {
        ...
        //筛选搜索返回的info
        NotificationCenter.default.addObserver(forName: NSNotification.Name("purchaseOrder"), object: nil, queue: nil) { (info) in
...
extension PurchaseOrderFormVC {
    fileprivate func getData (_ firstGet:Bool) {
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            PurchaseTool.SearchPurchOrder(params: params as? [String:String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    self.purchaseArray = array
                    if self.firstSaveArray.count == 0 {self.firstSaveArray = array}
                }else{
                    self.purchaseArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let VC = purchaseDetailVC()
        VC.purchaseModel = purchaseArray[indexPath.row]
...
extension PurchaseOrderFormVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = purchaseArray[indexPath.row]
        cell.clientNameLabel.text = model.SupplierName
        cell.contactLabel.text = model.PurchOrderNo
...
extension PurchaseOrderFormVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .purchase
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.purchaseArray = self.firstSaveArray
