...
class SupplierInfoVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var supplierArray = [SupplierModel]()
    lazy var firstSaveArray = [SupplierModel]()
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(forName: NSNotification.Name("supplierInfo"), object: nil, queue: nil) { (info) in
...
extension SupplierInfoVC {
    fileprivate func getData (_ firstGet:Bool) {
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            PurchaseTool.SearchSupplier(params: params as? [String : String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    self.supplierArray = array
                    if self.firstSaveArray.count == 0 {self.firstSaveArray = array}
                }else{
                    self.supplierArray.append(contentsOf: array)
                }
...
extension SupplierInfoVC{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.detailType = .supplierInfo
        vc.supplierModel = supplierArray[indexPath.row]
        self.navigationController?.pushViewController(vc, animated: true)
    }
}
extension SupplierInfoVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .supplierInfo
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.supplierArray = self.firstSaveArray
