...
class PaymentOrderVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var paymentArray = [PaymentModel]()
    lazy var firstSaveArray = [PaymentModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("PaymentOrder"), object: nil, queue: nil) { (info) in
...
extension PaymentOrderVC {
    ...
    fileprivate func getData(_ firstGet:Bool){
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            FinanceTool.SearchBuyPay(params: params as? [String : String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    ...
                    self.paymentArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let VC = PaymentOrderDetailVC()
        VC.paymentModel = paymentArray[indexPath.row]
...
extension PaymentOrderVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = paymentArray[indexPath.row]
        cell.clientNameLabel.text = model.SupplierName
        cell.contactLabel.text = "结算账户:\(model.AccountName)"
        cell.phoneLabel.text = model.BuyPayNo
        cell.collectMoneyLabel.text = "实付:\(model.RealPayAmt)元"
...
extension PaymentOrderVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .PaymentOrder
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.paymentArray = self.firstSaveArray
