...
class SellReceOrderVC: BaseTableViewVC {
    var refreshCount = 1
    var searchtext = ""
    lazy var sellArray = [SearchSellModel]()
    lazy var firstSaveArray = [SearchSellModel]()
    ...
    override func viewDidLoad() {
        ...
        setupfooter()
        setUI()
        NotificationCenter.default.addObserver(forName: NSNotification.Name("SellReceOrder"), object: nil, queue: nil) { (info) in
...
extension SellReceOrderVC {
    ...
    fileprivate func getData(_ firstGet:Bool){
        if let userInfo = ZJKeyChain.load("userInfo") as? [String:String] {
            ...
            FinanceTool.SearchSellRece(params: params as? [String : String], Success: { (page, array) in
                self.page = page
                ...
                if firstGet == true {
                    self.sellArray = array
                    if self.firstSaveArray.count == 0 {self.firstSaveArray = array}
                }else{
                    self.sellArray.append(contentsOf: array)
                }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let VC = SellReceOrderDetailVC()
        VC.sellModel = sellArray[indexPath.row]
...
extension SellReceOrderVC{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let model = sellArray[indexPath.row]
        cell.clientNameLabel.text = model.ClientName
        cell.contactLabel.text = "结算账户:\(model.AccountName)"
        cell.phoneLabel.text = model.SellReceNo
        cell.collectMoneyLabel.text = "实收:¥\(model.RealReceAmt)"
...
extension SellReceOrderVC : UISearchResultsUpdating ,UISearchBarDelegate {
    
    
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        let searchRequestVC = SearchRequestVC()
        searchRequestVC.purchaseSearchType = .SellReceOrder
    ...
    func searchBarBookmarkButtonClicked(_ searchBar: UISearchBar) {
        ...
        self.sellArray = self.firstSaveArray
